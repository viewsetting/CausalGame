...
## GAME FLOW
1. **STAGE 1 (Exploration)**: Use `client.deploy_drone_v2()` to test hypotheses
...
{{#if deployment_budget}}
   - **BUDGET LIMIT**: You can only call `deploy_drone_v2` up to {{deployment_budget}} times!
{{/if}}
2. **STAGE 2 (Validation)**: Call `client.submit_final_design_v2(design)`
...
## AVAILABLE METHODS
- `client.get_mission_data()` - Get all flight history
...
- `client.get_status_v2()` - Current mission status
- `client.deploy_drone_v2(design, count=1, equipment=None)` - Deploy drones
  - design: {{design_schema}}
{{#if has_equipment}}  - equipment (optional): {{equipment_schema}}
{{/if}}- `client.submit_final_design_v2(design, equipment=None)` - **FINAL** submission (one-time only!)
